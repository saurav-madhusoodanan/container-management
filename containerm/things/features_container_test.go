...
func TestFeatureOperationsHandlerRemove(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"test_feature_operations_handler_remove_no_errors": {
			opts: testValidUnmarshaled,
			mockExecution: func() error {
				mockContainerManager.EXPECT().Remove(gomock.Any(), testContainerID, true, false, nil).Times(1)
				return nil
			},
		},
		...
		"test_feature_operations_handler-remove_error_while_removing": {
			...
			mockExecution: func() error {
				...
				mockContainerManager.EXPECT().Remove(gomock.Any(), testContainerID, true, false, nil).Times(1).Return(err)
				return err
			},
		},
	}
...
func TestFeatureOperationsUnsupportedOperation(t *testing.T) {
	...
	mockContainerManager.EXPECT().Remove(gomock.Any(), testContainerID, true, false, nil).Times(0)
